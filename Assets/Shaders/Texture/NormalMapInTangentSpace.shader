		...
		SubShader{
				Pass {
					...
					fixed4 frag(v2f i) : SV_Target{
						...
						fixed3 diffuse = _LightColor0.rgb * albedo * (0.5 + 0.5*dot(i.lightDir, tangentNormal));
						fixed3 halfDir = normalize(i.viewDir + i.lightDir);
						fixed3 specular = _LightColor0.rgb * _Specular.rgb * pow(0.5 + 0.5*dot(tangentNormal, halfDir), _Gloss);
